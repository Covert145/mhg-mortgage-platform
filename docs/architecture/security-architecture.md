...
1. **PostgreSQL Row-Level Security** — every tenant-scoped table has a policy restricting rows to `organization_id = current_setting('app.current_org_id')`, set per request/transaction before any query runs. This is the hard backstop that holds even if application-layer scoping has a bug. Two tables were moved out of this policy during implementation once the "which organization does this belong to" chicken-and-egg problem became concrete: `org_memberships` (a user must be able to discover which organizations they belong to *before* any org context exists — the same reasoning that already put `users`/`accounts`/`sessions` outside RLS, see below) and `invites` (accepting an invite is a public, unauthenticated flow with no session to derive an org from; the random 256-bit token itself is the access-control artifact — every lookup filters on the unique token column, never lists invites without one). Both are documented inline at `packages/db/prisma/migrations/20260815195859_org_membership_identity_lookup` and `.../20260815200140_invite_token_lookup`, and both remain governed by explicit application-layer filtering (every query includes the relevant `userId`/`token` predicate) rather than a table-level policy.
