...
- **Session**: Auth.js database session strategy; the session references `userId` plus an `activeOrganizationId` for multi-org users (chosen at login or via an org switcher); the cookie carries only the session token — role/permission claims are re-read from `OrgMembership` server-side on every authorization check, so a revoked or changed permission takes effect immediately rather than waiting for a JWT to expire.
