...
| Auth | Auth.js (NextAuth v5), JWT session strategy | Clerk, WorkOS, Supabase Auth | Full control of org/role/branch claims, needed for a platform whose core value is precise multi-tenant/RBAC control over mortgage data; avoids syncing our org model into a third-party auth system. JWT rather than database sessions because Auth.js's Credentials provider does not support database sessions (see `docs/architecture/security-architecture.md` #1) — permission checks still always re-read from the database, only session revocation is JWT-lifetime-bound |
